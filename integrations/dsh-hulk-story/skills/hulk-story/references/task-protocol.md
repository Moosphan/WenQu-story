...
Author tasks receive `required_memory` and bounded `supplementary_memory`. Required memory contains unresolved promises plus memories linked to named chapter participants or exact plan terms; it must not be discarded for relevance ranking. Supplemental memory is deterministic lexical recall and may be empty. The legacy `canonical_memory` field currently mirrors required memory for host compatibility. A chapter outline may set `pov` to a canonical entity key. When it does, `knowledge` records owned by another character are withheld from the task; `pov_context` reports only the selected POV and a withheld count, never the withheld keys or values. No `pov` means legacy behavior remains unchanged.
