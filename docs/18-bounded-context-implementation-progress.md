...
本文件保留历次实施记录；当前状态以紧接下方的“2026-09-19 完成批次”及最新验收记录为准。早期各批次的环境边界和测试数量只适用于当时。

## 2026-09-19 完成批次

本批已实现剩余工程入口：按模型绑定本地 tokenizer、实体合并与重定向、作者确认的依赖修复、章/卷语义摘要提案与核实、伏笔区间与关系触发、独立正文分支、本地向量/HNSW 与混合检索、首次自适应任务的有界检索，以及四策略评测和完整状态机实验工具。新增作者操作提供 Core、HTTP 与 CLI；现有 GUI 不包含这些高级操作的专用表单。使用方法见 [长期记忆操作手册](19-bounded-context-operations.md)。

SQLite 增量升级至 10；7/8/9 的来源、正文和稳定 ID 保留。实体合并不改正文；依赖修复必须逐项替换或退休旧事件，不能把过期来源重新认证。摘要必须有逐章来源并由作者明确核实，来源失效后不会因事实修复而自动复活。分支通过新作品 ID 和独立版本指针隔离，父作品删除不删除子分支。

首次 adaptive 输入从索引取有界候选、直接查显式硬依赖、读取近期三章，避免把全部历史正文和记忆先装入 Python。SQL 的检索与状态投影仍随资料增长，不声称常数复杂度。向量编码发生在写事务外，语义索引维护每次只执行一个有界批次，查询仍复核作品、当前版本、章界和 POV。默认使用 lexical；未配置本地模型时语义评测明确为 unavailable。

已完成 500 章真实服务状态机模拟：2,602 次 `next_task`/`submit_task`，包括 500 次初稿、500 次抽取、500 次连续性审查、1,000 次读者审查、99 次章弧和最终检查。100/300/500 章检查点的完整任务输入分别为 5,602–15,745 / 5,602–15,778 / 5,602–15,404 字节（包含诊断字段，非服务商账单）。预算和返修上限守卫均正确停止。结果由确定性合成适配器提供，模型生成调用为 0；超过 200 章仅使用实验库 fixture，产品章数上限不变。

off 对照同样完成 2,602 次任务，500 章检查点峰值 146,375 字节。合成正文重复度高，前期 canonical 去重后资料很少；不能据此声称一般小说的压缩率或费用降幅。adaptive 预留 70,452,790、off 预留 52,495,246，预留算法不同且不是实际花费，不能互相当作账单比较。

另只读分析了已授权作品现有用量记录：53 个有效输入校准样本，flash 29、pro 24，在保守字节估算下低估次数均为 0。该样本不证明上下文容量或隐性开销上界，也没有据此自动缩小余量。可选本地 tokenizer 仍计数序列化请求估算，未冒充服务商内部模板的精确计数。

工程交付不等于所有质量指标已获实证：普通中文语义召回率、真实生成成本、文风衔接仍需明确模型工件及独立标注样本评测。测试使用的随机微型本地模型仅验证无网络编码和索引链路，不能用于宣称中文语义质量。本批没有增加真实模型调用。

最终工程验证：558 项 pytest 通过（2 项既有 Web 依赖弃用警告、1 项可选语义测试依赖弃用警告），JavaScript 语法与差异检查通过。独立复审发现的状态页语义查询、租约竞态、空书语义索引启动、无效关系引用、索引删除残留及评测统计单位问题均已补回归并修复。

本批已部署到 8765。部署前确认无生成任务运行，备份数据库及旧源码到原书库 `backups/memory-completion-20260919-175254`。重启后首页与作品状态接口正常，SQLite 10 完整性为 ok、外键违规为 0；全部 33 个正文历史版本内容摘要与备份一致，62 个运行文件与已测试工作区逐字节相同。目标作品仍暂停在 19 章定稿、revision 31，没有触发续写或新模型消费。保留原上下文配置，不自动启用未配置的语义模型。
...
以下为早期批次账本，保留当时缺项供追溯；其中工程缺项现已由上方完成批次实现。真实 500 章模型生成、普通同义线索 95% 召回或文学质量提升仍没有证据。
